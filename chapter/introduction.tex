...
しかしこれらの集団は分子集団とは全く異なる振る舞いを見せる。例えば\figref{fig:example_actmat}
...
そのような現象の１つとして、\figref{fig:intro_confine}のようにアクティブマターを円形領域の中に入れると、その
円の中に渦が発生することが知られている。
この渦は、円の半径と系が持つ相関長が等しくなる時、
\figref{fig:intro_confine}(a)、(b) のように１つになることが分かっている。
...
%TODO:図の追加：シミュレーションのconfine%流体と壁の例：
...
入っていないシンプルなモデルである。%TODO:その性質からあと２、３行
そのシンプルさにも関わらず、
...
\begin{figure}
    ...
    \begin{tabular}{c}
        ...
        \begin{minipage}{0.4\hsize}
            \text{(b)}
            \includegraphics[width=\textwidth]{img/intro/fig_cabp_orientation.png}
        \end{minipage}
    \end{tabular}
    \caption[Four sample images]
    {
...
において観測されている。\figref{fig:intro_abp_cabp_velfield}はこれらの系における速度の配向のスナップショットで、その速度に相関が見られる。
